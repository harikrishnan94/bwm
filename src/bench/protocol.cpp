...
Expected<TimedPhaseResult> run_timed_phase(const PhaseConfig& cfg,
  ...
  try {
    ...
    if (warmup.count() > 0) {
      auto warm_res = bwm::app::run_phase_for_protocol(app_cfg, phase_mode, warmup, 0, true);
      if (!warm_res) {
        return std::unexpected(warm_res.error());
      }
    }

    auto timed = bwm::app::run_phase_for_protocol(app_cfg, phase_mode, duration, 0, false);
    if (!timed) {
    ...
    return std::unexpected(Error{ErrorCode::Internal, std::string("run_timed_phase exception: ") + ex.what()});
  } catch (...) {
    return std::unexpected(Error{ErrorCode::Internal, "run_timed_phase unknown exception"});
  }
}
